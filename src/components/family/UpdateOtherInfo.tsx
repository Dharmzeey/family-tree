...
import { updateOtherInfo } from "@/lib/validation/family";
...
export default function UpdateOtherInformation({ otherInformation }: props) {
    const [formState, formAction] = useActionState(updateOtherInfo, initialState)
        ...
        <>
            <div className="bg-[#ffffff0a] shadow-xl backdrop-blur-md w-9/12 lg:w-1/2 py-8 px-4 rounded-2xl">
                <form action={formAction} className="flex flex-col justify-center items-center w-full">
                    <h2 className="text-2xl">Update Other Information</h2>
                    ...
                    />
                    <SubmitButton pendingText="Updating..." buttonText="UPDATE OTHER INFOTMATION" />
                    {formState.error && (
