...
import { updateEulogy } from "@/lib/validation/family";
...
export default function UpdateEulogy({ eulogy }: props) {
    const [formState, formAction] = useActionState(updateEulogy, initialState)
        ...
        <>
            <div className="bg-[#ffffff0a] shadow-xl backdrop-blur-md w-9/12 lg:w-1/2 py-8 px-4 rounded-2xl">
                <form action={formAction} className="flex flex-col justify-center items-center w-full">
                    <h2 className="text-2xl">Update Eulogy</h2>
                    ...
                    />
                    <SubmitButton pendingText="Updating..." buttonText="UPDATE EULOGY" />
                    {formState.error && (
